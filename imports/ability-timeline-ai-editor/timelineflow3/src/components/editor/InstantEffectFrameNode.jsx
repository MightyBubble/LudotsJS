...
import { Sparkles, User, Crosshair, Clock, Activity, MousePointer2 } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function InstantEffectFrameNode({ 
  clip,
  isSelected, 
  onSelect, 
  onDrag, 
  ...
  const dataFields = [
    { type: 'effectType', label: 'Type', value: clip.effectType || 'Damage', icon: Sparkles, isText: true },
    { type: 'value', label: 'Value', value: clip.value || 100, icon: Activity }
  ];
    ...
    >
      <div className="flex flex-row items-stretch h-full min-h-[100px]">
        <div className="w-6 flex flex-col items-center bg-[#1a1d23] border-r border-white/5 rounded-l-lg z-10 relative pt-3">
          ...
          >
            {renderExecutionPin('exec', false, isInputSnapTarget('exec'), hasInputConnection('exec'), '#f59e0b')}
          </div>
        </div>

        <div className="flex-1 flex flex-row relative h-full p-1">
          <div className="absolute top-2 left-2 pointer-events-none z-0">
            <span className="text-[9px] font-bold text-amber-500/50 tracking-wider">EXEC</span>
          </div>

          <div className="flex flex-col justify-center gap-1 pl-6 pr-4 py-4 min-w-[120px] z-10">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-gradient-to-br from-amber-500/20 to-amber-600/5 rounded-md border border-amber-500/20">
            ...
            <span className="text-[9px] text-gray-500 font-mono select-none">Instant Frame</span>
          </div>
          ...
          <div className="flex flex-1 gap-2 pr-2 py-1 items-stretch z-10">
            ...
            {renderSection('Data', 'bg-amber-500', 'border-amber-500', 'text-amber-500', dataFields)}
          </div>
        </div>
        ...
        <div className="w-6 flex flex-col justify-center items-center bg-[#1a1d23] border-l border-white/5 rounded-r-lg relative">
          ...
          >
            {renderExecutionPin('execOut', true, isOutputSnapTarget('execOut'), hasOutputConnection('execOut'), '#f59e0b')}
          </div>
        </div>
      </div>
    </div>
