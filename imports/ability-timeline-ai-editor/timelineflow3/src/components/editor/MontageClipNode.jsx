...
import { Clapperboard, User, Clock, Timer, Film, Play } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function MontageClipNode({ 
  clip,
  isSelected, 
  onSelect, 
  onDrag, 
  ...
  const contextFields = [
    { type: 'caster', label: 'Character', icon: User }
  ];
  ...
  const dataFields = [
    { type: 'animName', label: 'Anim', value: clip.animName || '', icon: Film, isText: true },
    { type: 'playRate', label: 'Rate', value: clip.playRate || 1.0, icon: Play }
  ];
    ...
    >
      <div className="flex flex-row items-stretch h-full min-h-[110px]">
        <div className="w-6 flex flex-col items-center bg-[#1a1d23] border-r border-white/5 rounded-l-lg z-10 relative pt-3">
          ...
          >
            {renderExecutionPin('exec', false, isInputSnapTarget('exec'), hasInputConnection('exec'), '#78716c')}
          </div>
        </div>

        <div className="flex-1 flex flex-row relative h-full p-1">
          <div className="absolute top-2 left-2 pointer-events-none z-0">
            <span className="text-[9px] font-bold text-stone-500/50 tracking-wider">EXEC</span>
          </div>

          <div className="flex flex-col justify-center gap-1 pl-6 pr-4 py-4 min-w-[120px] z-10">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-gradient-to-br from-stone-500/20 to-stone-600/5 rounded-md border border-stone-500/20">
            ...
            <span className="text-[9px] text-gray-500 font-mono select-none">Montage Clip</span>
          </div>
          ...
          <div className="flex flex-1 gap-2 pr-2 py-1 items-stretch z-10">
            ...
            {renderSection('Data', 'bg-stone-500', 'border-stone-500', 'text-stone-500', dataFields)}
          </div>
        </div>
        ...
        <div className="w-8 flex flex-col items-center justify-center bg-[#1a1d23] border-l border-white/5 rounded-r-lg relative">
          <div className="flex flex-col items-center gap-2">
            <div className="absolute top-2 -right-4 rotate-90 origin-top-left pointer-events-none whitespace-nowrap">
              <span className="text-[8px] font-bold text-purple-500/50 tracking-widest">CALLBACK</span>
            </div>

            <div 
              ref={(el) => pinRefs.current[`onCompleted-output`] = el}
              className="pin-handle cursor-crosshair flex items-center justify-center w-6 h-6 hover:scale-110 transition-transform z-50"
              onMouseDown={(e) => { e.stopPropagation(); onPinMouseDown(e, clip.id, 'onCompleted', true, true); }}
            >
              {renderExecutionPin('onCompleted', true, isOutputSnapTarget('onCompleted'), hasOutputConnection('onCompleted'), '#a855f7')}
            </div>
          </div>
        </div>
      </div>
