...
import { Layers, User, Crosshair, Clock, Timer, MousePointer2, ExternalLink } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function CompositeClipNode({ 
  clip,
  isSelected, 
  onSelect, 
  onDrag, 
    ...
    >
      <div className="flex flex-row items-stretch h-full min-h-[110px]">
        ...
        <div className="flex-1 flex flex-row relative h-full p-1">
          ...
          <div className="flex flex-col justify-center gap-1 pl-6 pr-4 py-4 min-w-[100px] z-10">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-gradient-to-br from-emerald-500/20 to-emerald-600/5 rounded-md border border-emerald-500/20">
                <Layers className="w-4 h-4 text-emerald-400" />
              </div>
              <span className="text-sm font-bold text-gray-200 uppercase tracking-wide text-shadow-sm whitespace-nowrap overflow-hidden text-ellipsis select-none flex-1">{clip.name}</span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onDiveIn?.(clip.id);
                }}
                className="p-1 hover:bg-white/10 rounded text-gray-500 hover:text-emerald-400 transition-colors flex-shrink-0"
                title="进入子层级"
              >
                <ExternalLink className="w-3 h-3" />
              </button>
            </div>
            <span className="text-[9px] text-gray-500 font-mono select-none">Container</span>
          </div>
