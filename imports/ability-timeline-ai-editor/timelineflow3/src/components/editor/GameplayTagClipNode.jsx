...
import { Tag, User, Crosshair, Clock, Timer, MousePointer2, Box } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function GameplayTagClipNode({ 
  clip,
  isSelected, 
  onSelect, 
  onDrag, 
  ...
  const dataFields = [
    { type: 'attachmentCount', label: 'Attach', value: clip.attachmentCount || 0, icon: Box }
  ];
    ...
    >
      <div className="flex flex-row items-stretch h-full min-h-[110px]">
        <div className="w-6 flex flex-col items-center bg-[#1a1d23] border-r border-white/5 rounded-l-lg z-10 relative pt-3">
          ...
          >
            {renderExecutionPin('exec', false, isInputSnapTarget('exec'), hasInputConnection('exec'), '#ec4899')}
          </div>
        </div>

        <div className="flex-1 flex flex-row relative h-full p-1">
          <div className="absolute top-2 left-2 pointer-events-none z-0">
            <span className="text-[9px] font-bold text-pink-500/50 tracking-wider">EXEC</span>
          </div>

          <div className="flex flex-col justify-center gap-1 pl-6 pr-4 py-4 min-w-[120px] z-10">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-gradient-to-br from-pink-500/20 to-pink-600/5 rounded-md border border-pink-500/20">
            ...
            <span className="text-[9px] text-gray-500 font-mono select-none">Tag Clip</span>
          </div>
          ...
          <div className="flex flex-1 gap-2 pr-2 py-1 items-stretch z-10">
            ...
            {renderSection('Data', 'bg-pink-500', 'border-pink-500', 'text-pink-500', dataFields)}
          </div>
        </div>
      </div>
    </div>
