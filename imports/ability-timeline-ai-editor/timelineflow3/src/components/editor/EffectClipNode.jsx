...
import { Trash2, Zap, ChevronDown, ChevronRight, User, Crosshair, Clock, Activity, Play, Timer, MousePointer2, Box, Plus, Check, X } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function EffectClipNode({ 
  clip,
  isSelected, 
  onSelect, 
  onDrag, 
  ...
  const contextFields = [
    ...
    { type: 'casterPos', label: 'CasterPos', icon: MousePointer2 },
    { type: 'targetPos', label: 'TargetPos', icon: MousePointer2 }
  ];

  const timeFields = [
    { type: 'startTime', label: 'Start', value: clip.startTime, icon: Clock, hasInput: true },
    { type: 'duration', label: 'Duration', value: clip.duration, icon: Timer, hasInput: true },
    { type: 'endTime', label: 'End', value: clip.endTime, icon: Clock, hasInput: false }
  ];

  const dataFields = [
    { type: 'magnitude', label: 'Magnitude', value: clip.magnitude || 0, icon: Activity },
    { type: 'stackCount', label: 'Stack', value: clip.stackCount || 1, icon: Box }
  ];

  const lifecycleCallbacks = [
    { type: 'onTryAdd', label: 'OnTryAdd', color: '#64748b', icon: Play },
    { type: 'onApplied', label: 'OnApplied', color: '#10b981', icon: Play },
    { type: 'onTick', label: 'OnTick', color: '#3b82f6', icon: Timer },
    { type: 'onInterrupted', label: 'OnInterrupted', color: '#f59e0b', icon: Activity },
    { type: 'onTimeout', label: 'OnTimeout', color: '#8b5cf6', icon: Clock },
    { type: 'onRemoved', label: 'OnRemoved', color: '#ef4444', icon: Trash2 }
  ];

  // Legacy callback logic removed
  ...
  // Helper for rendering grouped sections
        ...
        <div className="flex gap-4 items-start h-full mt-1">
          {fields.map((field) => (
            <div key={field.type} className="flex flex-col items-center justify-between gap-2 h-full min-w-[40px] relative group">
              ...
              {/* Input Pin Socket - only for fields with hasInput */}
      ...
      <div className="flex flex-row items-stretch h-full min-h-[110px]">
        ...
        <div className="w-6 flex flex-col items-center bg-[#1a1d23] border-r border-white/5 rounded-l-lg z-10 relative pt-3">
          <div 
            ...
            onMouseDown={(e) => {
              e.stopPropagation();
              onPinMouseDown(e, clip.id, 'exec', false, true);
            }}
          >
            {renderExecutionPin('exec', false, isInputSnapTarget('exec'), hasInputConnection('exec'), '#10b981')}
          </div>
        </div>

        {/* CENTER: Parameters & Header */}
        <div className="flex-1 flex flex-row relative h-full p-1">
          {/* Execution Label - Horizontal */}
          <div className="absolute top-2 left-2 pointer-events-none z-0">
             <span className="text-[9px] font-bold text-emerald-500/50 tracking-wider">EXEC</span>
          </div>

          {/* Header Section */}
          <div className="flex flex-col justify-center gap-1 pl-6 pr-4 py-4 min-w-[120px] z-10">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-gradient-to-br from-violet-500/20 to-violet-600/5 rounded-md border border-violet-500/20">
              ...
              <span className="text-[9px] text-gray-500 font-mono select-none">Effect Clip</span>
          </div>
          ...
          <div className="flex flex-1 gap-2 pr-2 py-1 items-stretch z-10">
            ...
            {renderSection('Data', 'bg-emerald-500', 'border-emerald-500', 'text-emerald-500', dataFields)}
          </div>
        </div>

        {/* RIGHT: Events Output */}
        <div className="w-8 flex flex-col items-center justify-center bg-[#1a1d23] border-l border-white/5 rounded-r-lg relative">
           <div className="flex flex-col items-center gap-2">
              <div className="absolute top-2 -right-4 rotate-90 origin-top-left pointer-events-none whitespace-nowrap">
                <span className="text-[8px] font-bold text-purple-500/50 tracking-widest">EVENTS</span>
              </div>

              <div 
                ref={(el) => pinRefs.current[`events-output`] = el}
                className="pin-handle cursor-crosshair flex items-center justify-center w-6 h-6 hover:scale-110 transition-transform z-50"
                onMouseDown={(e) => {
                  e.stopPropagation();
                  onPinMouseDown(e, clip.id, 'events', true, false);
                }}
              >
                {renderDataPin('events', true, isOutputSnapTarget('events'), hasOutputConnection('events'), '#a855f7')}
              </div>
           </div>
        </div>
