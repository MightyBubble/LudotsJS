  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="bg-[#15171C] border-[#2A2E37] text-gray-300 min-w-56">
        <DropdownMenuLabel className="text-[10px] uppercase text-gray-500">工作区</DropdownMenuLabel>
        {workspaces.map(w => (
          <DropdownMenuItem
            key={w.workspace_id}
        ...
        <DropdownMenuLabel className="text-[10px] uppercase text-gray-500">项目</DropdownMenuLabel>
        ...
        {projectsInWorkspace.length === 0 && (
          <DropdownMenuItem disabled className="text-xs text-gray-500">此工作区暂无项目</DropdownMenuItem>
        )}
      </DropdownMenuContent>
    </DropdownMenu>
  );
