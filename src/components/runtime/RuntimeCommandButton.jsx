...
export default function RuntimeCommandButton({ button, onActivate }) {
  ...
  const accent = ability.accentColor || '#cbd3dc';
  const hotkey = (button.action_id || '').split('.').pop();
  return (
    <button
      onClick={() => onActivate(button)}
      title={[ability.hintText, ...button.trace].filter(Boolean).join('\n')}
      className="relative aspect-square rounded bg-[#1E2128] border border-[#424a55] hover:border-[#cbd3dc] transition-colors flex flex-col items-center justify-center gap-1 p-1"
