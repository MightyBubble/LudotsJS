...
import PresenterMiniStateMachine from './PresenterMiniStateMachine';
import { buildAnimatorPreviewLayer } from '@/lib/runtime/animatorPreview';

export default function PresenterAnimationPreviewControls({ draft, controllers, profiles, stateIndex, onStateIndex }) {
  ...
  return <Section title="Animator 预览" right={<Button size="sm" onClick={() => setPlaying(value => !value)} className={`h-7 text-xs ${playing ? 'bg-emerald-600' : 'bg-[#242a32]'}`}>{playing ? 'Stop' : 'Play'}</Button>}>
    <PresenterMiniStateMachine layer={layer} activeStateId={playing ? runtime.activeStateId : layer.states.find(state => state.packed_state_index === stateIndex)?.id} activeTransitionId={runtime.activeTransitionId} onSelect={onStateIndex} />
