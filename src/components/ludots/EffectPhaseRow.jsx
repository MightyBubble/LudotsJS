...
export default function EffectPhaseRow({ phase, config, handler, onChange, graphs }) {
  const mainLabel = handler ? `${handler.builtin ? 'Builtin Graph' : 'Graph'} · ${handler.id}` : 'None';
  return (
    <div className="grid gap-3 border border-[#2A2E37] rounded p-3 md:grid-cols-[140px_1fr_180px_1fr]">
      ...
      <div><div className="text-[11px] text-gray-400 mb-1">Main（Preset）</div><div className={`h-8 px-2 rounded border flex items-center gap-2 text-[11px] ${handler ? 'border-[#59616d] bg-[#242a32] text-[#dce2e8]' : 'border-[#2A2E37] text-gray-600'}`}><span className="truncate flex-1">{mainLabel}</span>{handler && <Link to={`/UnifiedGraphEditor?type=action&graph=${encodeURIComponent(handler.id)}`} title="编辑 Main ActionGraph"><ExternalLink className="w-3.5 h-3.5" /></Link>}</div><BoolField label="Skip Main" value={config.skipMain} onChange={(skipMain) => onChange({ ...config, skipMain })} /></div>
