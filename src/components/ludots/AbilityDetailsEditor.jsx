...
export default function AbilityDetailsEditor({ draft, patch, refs, issues }) {
  ...
  return <div className="max-w-6xl">
    ...
    {domain === 'execution' && <AbilityExecEditor value={draft.exec || {}} onChange={exec => patch({ exec })} />}
    {domain === 'rules' && <><AbilityActivationEditor draft={draft} patch={patch} refs={refs} /><AbilityOptionalEditor draft={draft} patch={patch} domain="state" /></>}
    {domain === 'targeting' && <AbilityOptionalEditor draft={draft} patch={patch} domain="targeting" />}
