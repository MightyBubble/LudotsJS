...
import { getGlobalTableOptions } from './globalTableRefs';
...
export default function EffectFieldControl({ field, value, onChange, refs }) {
  ...
  if (field.type === 'graph') return <EffectGraphSelect label={field.label} value={value} onChange={onChange} graphs={refs.actionGraphs} />;
  if (field.type === 'globalRef') return <SelectField label={field.label} value={value} options={getGlobalTableOptions(refs.constants, field.tableId)} onChange={onChange} />;
