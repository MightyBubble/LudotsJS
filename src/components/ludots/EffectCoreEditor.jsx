...
import { getEffectPresetDefinition, getEffectPresetOptions, presetPatch } from './effectPresetDefinitions';

export default function EffectCoreEditor({ draft, patch, refs }) {
  const preset = getEffectPresetDefinition(draft.presetType, refs.constants);
  ...
  return (
    <>
      <Section title="基础 Basic · EffectTemplateConfig">
        ...
        <SelectField label="presetType" value={draft.presetType || 'None'} options={getEffectPresetOptions(refs.constants)} onChange={(presetType) => patch(presetPatch(draft, presetType, refs.constants))} />
