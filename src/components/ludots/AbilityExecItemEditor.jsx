...
import { getGlobalTableValues, LUDOTS_TABLE_IDS } from './globalTableRefs';
...
export default function AbilityExecItemEditor({ item, index, callerParams = [], refs = {}, onChange, onRemove }) {
  ...
  const kinds = getGlobalTableValues(refs.constants, LUDOTS_TABLE_IDS.execItemKinds, EXEC_ITEM_KINDS);
  const clocks = getGlobalTableValues(refs.constants, LUDOTS_TABLE_IDS.gasClockIds, ['FixedFrame', 'Step', 'EntityLocal']);
  const dispatchTargets = getGlobalTableValues(refs.constants, LUDOTS_TABLE_IDS.dispatchTargets, ['Default', 'Source', 'Target', 'TargetContext']);
  return <div className="space-y-3">
    ...
    <AbilityFieldDomain title="节点定义" meta="Definition" description="决定 Loader 使用哪一种原生 Exec Item。"><SelectField label="类型 kind" value={kind} options={options(kinds)} onChange={value => patch({ kind: value })} /></AbilityFieldDomain>
    <AbilityFieldDomain title="时间域" meta="Timing" description="只负责时间轴位置、持续时间与时钟覆盖。"><NumberField label="时间点 tick" value={item.tick} onChange={tick => patch({ tick })} />{(clip || item.duration != null) && <NumberField label="持续 tick duration" value={item.duration} onChange={duration => patch({ duration })} />}<SelectField label="覆盖时钟 clock" value={item.clock} options={options(clocks)} onChange={clock => patch({ clock })} /></AbilityFieldDomain>
    ...
    {kind !== 'End' && <AbilityFieldDomain title="上下文域" meta="Dispatch" description="决定载荷映射到 Source、Target 或 TargetContext。"><SelectField label="分发目标 dispatchTarget" value={item.dispatchTarget} options={options(dispatchTargets)} onChange={dispatchTarget => patch({ dispatchTarget })} /></AbilityFieldDomain>}
