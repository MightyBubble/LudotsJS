...
export default function AbilityExecItemEditor({ item, index, callerParams = [], onChange, onRemove }) {
  ...
  return <div className="space-y-3">
    ...
    {kind !== 'End' && <AbilityFieldDomain title="载荷域" meta="Payload" description="Clip / Signal 的业务载荷，不包含透传参数。">{showTemplate && <TextField label="Effect 模板 template" value={item.template} onChange={template => patch({ template })} />}{showTag && <TextField label="标签 / 事件 tag" value={item.tag} onChange={tag => patch({ tag })} />}{showGraph && <TextField label="Graph 程序 graph" value={item.graph} onChange={graph => patch({ graph })} />}{showPayload && <NumberField label="载荷 payloadA" value={item.payloadA} onChange={payloadA => patch({ payloadA })} />}</AbilityFieldDomain>}
