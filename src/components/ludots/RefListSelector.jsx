  ...
  return (
    <div>
      ...
      <div className="space-y-1 mb-2">
        ...
        {(value || []).map(id => (
          <div key={id} className="flex items-center justify-between bg-[#0D0F14] border border-[#2A2E37] rounded px-2 py-1">
            <span className="text-[11px] text-gray-300">{options.find(o => o.value === id)?.label || id}</span>
      ...
      <Select value="" onValueChange={(v) => onChange([...(value || []), v])}>
        <SelectTrigger className="bg-[#0D0F14] border-[#2A2E37] text-[#e5e5e5] h-7 text-[11px]">
