...
export function buildEffectBuiltinGraph(actionId, name, operationType) {
  const entry = { id: 'effect-entry', type: 'action_entry', position: { x: 80, y: 180 }, data: { label: 'Effect Phase Context' }, inputs: [], outputs: ENTRY_OUTPUTS };
  if (actionId === 'Graph.Lifecycle.DeployConsumeSource') {
    const begin = { id: 'begin', type: 'runtime_builtin', position: { x: 340, y: 180 }, data: { label: 'BeginLifecycleTransaction' }, inputs: EXEC_INPUT, outputs: EXEC_OUTPUT };
    const handlers = DEPLOY_HANDLERS.map((handler, index) => ({ id: `builtin-${index}`, type: 'runtime_builtin', position: { x: 600 + index * 260, y: 180 }, data: { label: `InvokeBuiltin · ${handler}` }, inputs: EXEC_INPUT, outputs: index === DEPLOY_HANDLERS.length - 1 ? [] : EXEC_OUTPUT }));
  ...
  const sources = (contract.configs || []).map((item, index) => ({ id: `config-${item.id}`, type: 'runtime_param_source', position: { x: 80, y: 360 + index * 170 }, data: { label: item.label, details: item.fields }, inputs: [], outputs: [{ id: item.id, label: item.id, type: 'any' }] }));
  if (contract.params?.length) sources.push({ id: 'merged-params', type: 'runtime_param_source', position: { x: 340, y: 360 }, data: { label: 'EffectConfigParams · merged', details: contract.params }, inputs: [], outputs: [{ id: 'mergedParams', label: 'mergedParams', type: 'any' }] });
  if (contract.runtime?.length) sources.push({ id: 'runtime-scope', type: 'runtime_param_source', position: { x: 600, y: 360 }, data: { label: 'BuiltinHandlerRuntimeScope', details: contract.runtime }, inputs: [], outputs: [{ id: 'runtimeScope', label: 'runtimeScope', type: 'any' }] });
