...
export default function PlaygroundToolbar({ maps, mapId, onMap, mapEntityCount, paused, onToggle, onClear, count, elapsed, templateName, participantView }) {
  return (
    <div className="h-11 shrink-0 border-b border-[#2A2E37] bg-[#15171C] flex items-center gap-2 px-3 overflow-x-auto">
      ...
      <span className="ml-auto text-[10px] text-gray-500 whitespace-nowrap">
        {templateName ? `待放置：${templateName}` : '未选择模板'} · 地图实体 {mapEntityCount} · 临时实体 {count} · 时间 {elapsed.toFixed(1)}s
      </span>
    </div>
  );
}