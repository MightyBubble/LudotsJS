...
export default function RuntimeCommandPanelDock({ result, onAbility, onClose }) {
  ...
  return <RuntimeDock title="Command Panel Preview" icon={Gamepad2} onClose={onClose} className="h-44 shrink-0">
    <div className="grid gap-1.5 p-2" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
      {(result?.buttons || []).map(button => <button key={button.slot_id} data-testid={`panel-button-${button.slot_id}`} disabled={button.empty} onClick={() => button.ability && onAbility(button.ability)} className="min-h-14 rounded border border-border bg-muted p-2 text-left hover:border-ring hover:bg-accent disabled:opacity-35">
