...
export default function AnimationLocatorList({ rows = [], assets, onChange }) {
  ...
  return <Section title="Animation Host Bindings" right={<Button size="sm" onClick={() => onChange([...rows, { backend_id: 'browser', asset_ref: '', variant: '' }])} className="h-7 bg-[#242a32]"><Plus className="h-3.5 w-3.5" />Binding</Button>}>
    ...
    {rows.map((row, index) => <AnimationLocatorRow key={`${row.backend_id}-${index}`} locator={row} assets={animationAssets} assetOptions={options} onChange={next => patch(index, next)} onRemove={() => onChange(rows.filter((_, i) => i !== index))} />)}
  </Section>;
}