...
export default function MapEntitiesEditor({ rows = [], prototypes, onChange }) {
  ...
  return <Section title="Map 实体" right={<Button size="sm" onClick={add} className="h-7 bg-[#1E2128] hover:bg-[#2A2E37]"><Plus className="w-3 h-3" />添加实体</Button>}>
    ...
    <div className="space-y-2">{rows.map((row, index) => <div key={index} className="rounded border border-[#2A2E37] bg-[#0D0F14] p-3 space-y-2">
      <div className="grid grid-cols-[1fr_1fr_100px_100px_auto] gap-2 text-[10px] text-gray-500"><span><b className="text-gray-300">27</b> Instance ID</span><span><b className="text-gray-300">28</b> Template</span><span className="col-span-2"><b className="text-gray-300">29</b> Position X / Y</span></div>
      ...
      <p className="text-[10px] text-gray-500"><b className="text-gray-300">30</b> Overrides: {JSON.stringify(row.overrides || {})} · <b className="text-gray-300">31</b> Performer Params: {(row.performer_param_overrides || []).length}</p>
    </div>)}</div>
  </Section>;
}