...
export default function ControlPlaneEditorPage() {
  ...
  return <RecordWorkspace entityName="ControlPlaneProfile" records={editor.records} hideBrowserOnMobile
    columns={[{ key: 'control_plane_id', label: 'Control Plane ID', width: 280 }, { key: 'entity_query_graph_ref', label: 'Entity Query Graph' }]}
    toItem={record => ({ id: record.id, name: record.control_plane_id, subtitle: record.entity_query_graph_ref || '未选择查询图' })}
    ...
    {editor.draft && <ControlPlaneDetails draft={editor.draft} patch={editor.patch} queryGraphs={queryGraphs} />}
  </RecordWorkspace>;
}