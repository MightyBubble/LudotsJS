    ...
    <div className="h-screen flex flex-col bg-[#1e1e1e] text-white">
      <div className="h-12 bg-[#2d2d2d] border-b border-[#3d3d3d] flex items-center px-4 justify-between">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
          <TabsList className="bg-[#1e1e1e] border border-[#3d3d3d] h-8">
            <TabsTrigger value="definitions" className="text-xs px-3 data-[state=active]:bg-[#0e639c] data-[state=active]:text-white">
              <Settings2 className="w-3 h-3 mr-2" />
              关系定义 (Definitions)
            </TabsTrigger>
            <TabsTrigger value="static" className="text-xs px-3 data-[state=active]:bg-[#0e639c] data-[state=active]:text-white">
...
function RelationDefinitionsView() {
        ...
        {relation.attributes && relation.attributes.length > 0 && (
          <div className="flex flex-wrap gap-1">
            ...
            {relation.attributes.map(attrId => {
              ...
              return (
                <span key={attrId} className="bg-[#3d3d3d] px-1 rounded text-gray-300">
                  {attr?.name || attrId}
                </span>
              );
            })}
      ...
      <tr className="border-b border-[#3d3d3d] bg-[#252526]">
        <td className="p-2 align-top">
          <Input
            ...
            className="h-7 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white font-mono"
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
                <input
                  ...
                  className="rounded bg-[#1e1e1e] border-[#3d3d3d]"
                />
                有向关系
              </label>
              {data.is_directional && (
                 <Input
                   ...
                   className="h-6 w-32 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white"
                   placeholder="反向关系ID"
                 />
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-1 mb-1">
                {(data.attributes || []).map((attrId) => {
                   const attr = attributes.find(a => a.attribute_id === attrId);
                   return (
                    <div key={attrId} className="bg-[#3d3d3d] px-2 py-0.5 rounded text-xs flex items-center gap-1 text-gray-200">
        ...
        <td className="p-2 align-top">
          <div className="flex gap-1">
            <Button size="sm" onClick={handleSave} className="h-7 px-2 bg-[#0e639c] hover:bg-[#1177bb] text-xs">
              <Save className="w-3 h-3" />
            </Button>
            <Button size="sm" onClick={() => { setCreatingNew(false); setEditingId(null); setEditData(null); }} className="h-7 px-2 bg-[#3d3d3d] hover:bg-[#4d4d4d] text-xs">
              <X className="w-3 h-3" />
            </Button>
          </div>
        </td>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 bg-[#252526] border-b border-[#3d3d3d] flex justify-between items-center">
        ...
        <div className="flex gap-2">
           <div className="relative">
            ...
            <Input
              ...
              className="h-8 pl-8 w-48 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white"
            />
          </div>
          ...
          }} className="h-8 bg-[#3d3d3d] hover:bg-[#4d4d4d] text-white text-xs">
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-[#2d2d2d] border-b border-[#3d3d3d] text-left">
            <tr>
          ...
          <tbody>
            ...
            {filteredRelations.map(relation => {
              ...
              return (
                <tr key={relation.id} className="border-b border-[#3d3d3d] hover:bg-[#252526]">
                  ...
                  <td className="p-2">
                    <div className="flex gap-1">
                      <Button size="sm" onClick={() => { setEditingId(relation.id); setCreatingNew(false); setEditData({...relation}); }} className="h-6 w-6 p-0 bg-[#3d3d3d] hover:bg-[#4d4d4d]">
                        <Edit3 className="w-3 h-3" />
                      </Button>
                      <Button size="sm" onClick={() => { if(confirm('确认删除?')) deleteMutation.mutate(relation.id); }} className="h-6 w-6 p-0 bg-[#3d3d3d] hover:bg-[#5a1e1e]">
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-64 bg-[#252526] border-r border-[#3d3d3d] flex flex-col">
        <div className="p-3 border-b border-[#3d3d3d]">
          ...
          <Select value={selectedPrototypeId || ""} onValueChange={setSelectedPrototypeId}>
            <SelectTrigger className="w-full bg-[#1e1e1e] border-[#3d3d3d] text-white text-xs">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {prototypes.map(p => (
            <div 
              ...
              className={`p-2 rounded cursor-pointer text-xs flex justify-between items-center ${selectedPrototypeId === p.prototype_id ? 'bg-[#0e639c] text-white' : 'text-gray-300 hover:bg-[#3d3d3d]'}`}
            >
              <span className="truncate">{p.name}</span>
              {p.static_relations?.length > 0 && (
                <span className="bg-[#3d3d3d] px-1.5 rounded-full text-[10px] text-gray-400">{p.static_relations.length}</span>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 bg-[#1e1e1e] flex flex-col overflow-hidden">
        {selectedPrototype ? (
          <>
            <div className="p-4 border-b border-[#3d3d3d] bg-[#252526] flex justify-between items-start">
              <div>
              ...
              <Button onClick={handleAddRelation} className="bg-[#0e639c] hover:bg-[#1177bb] text-xs">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {(selectedPrototype.static_relations || []).length === 0 ? (
                <div className="text-center py-12 text-gray-500 border border-dashed border-[#3d3d3d] rounded-lg">
                ...
                (selectedPrototype.static_relations || []).map((rel, idx) => {
                  ...
                  return (
                    <Card key={idx} className="bg-[#252526] border-[#3d3d3d] shadow-none">
                      <CardContent className="p-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex gap-4">
                              <div className="flex-1">
                                ...
                                >
                                  <SelectTrigger className="h-8 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white">
                            ...
                            {relDef?.attributes?.length > 0 && (
                              <div className="bg-[#1e1e1e] p-3 rounded border border-[#3d3d3d]">
                                    ...
                                    return (
                                      <div key={attrId} className="flex flex-col gap-1">
                                        ...
                                        <Input
                                          ...
                                          className="h-7 bg-[#2d2d2d] border-[#3d3d3d] text-xs text-white"
                                        />
                                      </div>
                                    );
                                  })}
                          ...
                          <Button 
                            ...
                            className="text-gray-500 hover:text-red-400 hover:bg-[#3d3d3d] h-8 w-8 p-0"
                          >
