...
export default function InteractionEffectsPage() {
  ...
  const addEffectId = (value) => {
    if (!value.trim() || !editData) return;
    setEditData({
      ...editData,
      resulting_effect_ids: [...(editData.resulting_effect_ids || []), value.trim()]
    });
          ...
          <tbody>
            {creatingNew && editData && (
              <tr className="border-b border-[#2A2E37] bg-[#15171C]">
                <td className="p-2">
                  <Input value={editData.effect_name} onChange={(e) => setEditData({ ...editData, effect_name: e.target.value })} className="h-6 bg-[#0D0F14] border-[#2A2E37] text-xs text-white" />
                </td>
                <td className="p-2">
                  <Input value={editData.triggering_effect_tag_path} onChange={(e) => setEditData({ ...editData, triggering_effect_tag_path: e.target.value })} className="h-6 bg-[#0D0F14] border-[#2A2E37] text-xs text-white" list="tags-datalist" />
                </td>
                <td className="p-2">
                ...
                <td className="p-2">
                  <Input type="number" value={editData.priority} onChange={(e) => setEditData({ ...editData, priority: parseInt(e.target.value) || 0 })} className="h-6 bg-[#0D0F14] border-[#2A2E37] text-xs text-white w-16" />
                </td>
                <td className="p-2">
            ...
            {filteredEffects.map((effect) => {
              ...
              if (isEditing && editData) {
                return (
                  <tr key={effect.id} className="border-b border-[#2A2E37] bg-[#15171C]">
                    <td className="p-2">
                      <Input value={editData.effect_name} onChange={(e) => setEditData({ ...editData, effect_name: e.target.value })} className="h-6 bg-[#0D0F14] border-[#2A2E37] text-xs text-white" />
                    </td>
                    <td className="p-2">
              ...
              return (
                <tr key={effect.id} className="border-b border-[#2A2E37] hover:bg-[#15171C]">
                  ...
                  <td className="p-2">
                    <div className="flex gap-1">
                      <Button size="sm" onClick={() => handleEdit(effect)} className="h-6 w-6 p-0 bg-[#1E2128] hover:bg-[#2A2E37]">
                        <Edit3 className="w-3 h-3" />
                      </Button>
                      <Button size="sm" onClick={() => handleDelete(effect.id)} className="h-6 w-6 p-0 bg-[#1E2128] hover:bg-[#7f1d1d]">
