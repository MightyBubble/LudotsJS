      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 bg-[#252526] border-r border-[#3d3d3d] flex flex-col">
          <div className="p-3 border-b border-[#3d3d3d]">
            <span className="text-xs font-semibold text-gray-400">标签计数</span>
          </div>
          
          <div className="flex-1 overflow-auto p-2 space-y-1">
            {relevantTags.map(tag => {
              const tagInfo = tags.find(t => t.full_path === tag);
              const count = tagCounts[tag] || 0;
              return (
                <div key={tag} className="bg-[#1e1e1e] border border-[#3d3d3d] rounded p-2 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-white/90 truncate">{tagInfo?.name || tag.split('.').pop()}</div>
                    <div className="text-[9px] text-gray-500 font-mono truncate">{tag}</div>
                  </div>
                  <div className="flex items-center gap-1 ml-2">
                    <button
                      onClick={() => updateTagCount(tag, -1)}
