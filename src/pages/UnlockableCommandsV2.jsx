...
import { Search, Plus, Edit3, Trash2, X, Save, KeyRound, Eye, EyeOff } from "lucide-react";
  ...
  return (
    <div className="flex gap-1">
      <Input
        ...
        onBlur={onBlur}
        placeholder="输入或选择标签"
        className="h-7 bg-[#2d2d2d] border-[#3d3d3d] text-xs text-white flex-1"
        list="tags-datalist"
      />
      <Button
        size="sm"
...
function TagList({ tags, onRemove, canEdit }) {
  ...
  return (
    <div className="space-y-1">
      {tags.map((tag, idx) => (
        <div key={idx} className="flex items-center gap-1 bg-[#1e1e1e] px-2 py-1 rounded">
          <span className="text-xs text-gray-300 font-mono flex-1">{tag}</span>
          {canEdit && (
...
export default function UnlockableCommandsV2() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#1e1e1e] text-white">
      <div className="h-10 bg-[#2d2d2d] border-b border-[#3d3d3d] flex items-center px-4 gap-3">
        ...
        <span className="text-sm font-semibold text-gray-300">指令可见性管理器 V2</span>
        <span className="text-xs text-gray-500">管理指令的发现与可见 | 共 {filteredRules.length} 条</span>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-[#2d2d2d] border-b border-[#3d3d3d]">
            <tr>
              <th className="text-left p-2 font-semibold text-gray-300 w-40">规则名称</th>
              <th className="text-left p-2 font-semibold text-gray-300 w-48">解锁的指令标签</th>
              ...
              <th className="text-left p-2 font-semibold text-gray-300 w-16">状态</th>
          ...
          <tbody>
            {creatingNew && editData && (
              <tr className="border-b border-[#3d3d3d] bg-[#252526]">
                <td className="p-2">
                  <Input value={editData.rule_name} onChange={(e) => setEditData({ ...editData, rule_name: e.target.value })} className="h-6 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white" />
                </td>
                <td className="p-2">
                  <Input value={editData.unlocked_command_tag_path} onChange={(e) => setEditData({ ...editData, unlocked_command_tag_path: e.target.value })} className="h-6 bg-[#1e1e1e] border-[#3d3d3d] text-xs text-white" list="tags-datalist" />
                </td>
                <td className="p-2">
            ...
            {filteredRules.map((rule) => {
              ...
              return (
                <tr key={rule.id} className="border-b border-[#3d3d3d] hover:bg-[#252526]">
                  ...
                  <td className="p-2">
                    {rule.is_active ? <span className="text-gray-300">✓</span> : <span className="text-gray-600">-</span>}
                  </td>
                  <td className="p-2">
                    <div className="flex gap-1">
